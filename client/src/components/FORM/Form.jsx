...
function validation(input) {
  ...
  if (!input.name) {
    ...
    if (input.weight < 1 || input.weight > 82) {
      errors.weight = 'The weight must be between "1 kg" and "82 kg"';
    }
  } else if (!input.age) {
    errors.age = 'Age is required';
    if (input.age === 0 || input.age > 20) {
      errors.age = 'Age should be bigger than 0 and lower than 20 years';
    }
  } else if (!input.image) {
...
export default function Creator() {
  ...
  }, [dispatch, params.id, dogs, input.id]);
  ...
  function handleSubmit(e) {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      alert(`You are missing: ${Object.values(errors).join(', ')}`);
    }
  }
